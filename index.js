...
function loadData() {
  try {
    if (fs.existsSync(DATA_FILE)) {
      ...
      console.log(
        ` ✓  Data berhasil dimuat (${studentsData.length} siswa)`.cyan
      );
    } else {
...
function addGradeToStudent() {
  ...
  if (student.addGrade(subject, score)) {
    console.log(
      `\n✓ Nilai ${subject} (${score}) berhasil ditambahkan untuk ${student.name}!`
        .green.bold
    );
    saveData(); // Simpan data setelah operasi
    ...
    console.log(
      `  Status: ${
        student.getGradeStatus() === 'Lulus'
          ? student.getGradeStatus().green
          : student.getGradeStatus().red
      }`
    );
  } else {
...
function viewClassStatistics() {
  ...
  console.log(
    `  Rata-rata Kelas     : ${stats.classAverage.toFixed(2).yellow}`
  );
  console.log(
